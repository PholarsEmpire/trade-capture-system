...
public class TradeController {
   ...
   /* RSQL search endpoint for complex queries
    ...
    public ResponseEntity<?> searchByRsql(
            @RequestParam String query,
            @PageableDefault(size = 20, sort = "id") Pageable pageable) {
        try {
            Page<Trade> results = tradeService.searchByRsql(query, pageable);
            Page<TradeDTO> tradeDTOs = results.map(tradeMapper::toDto);
            return ResponseEntity.ok(tradeDTOs);
        } catch (Exception e) {
            logger.error("Error parsing RSQL query: {}", e.getMessage());
            return ResponseEntity.badRequest().body("Invalid RSQL query: " + e.getMessage());
        }
    }
    ...
    public ResponseEntity<List<TradeDTO>> getMyTrades() {
        ...
        //use the service to fetch/find trades by username/loginId
        var trades = tradeService.getTradesByTrader()
    ...
    @PreAuthorize("hasAnyRole('TRADER', 'TRADER_SALES', 'SUPERUSER', 'MIDDLE_OFFICE', 'MO', 'ADMIN')")
    ...
    @PreAuthorize("hasAnyRole('TRADER', 'TRADER_SALES', 'SUPERUSER', 'MIDDLE_OFFICE' , 'MO', 'ADMIN', 'SUPPORT' )")
